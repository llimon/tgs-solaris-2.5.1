...
| Header | Description |
| :--- | :--- |
| **`strtoimax_compat.h`** | Inline C99 64-bit string-to-integer parsers (`strtoimax`, `strtoumax`) featuring base-36 support, auto-base detection (hex/octal), and `ERANGE` bounds checking. Includes an embedded unit test runner (`_TEST_STRTOIMAX_CUSTOM`). |
...
| **`math_compat.h`** | Type-generic C99 floating-point classification macro (`isinf`) implemented via IEEE 754 bit-pattern inspection for float and double types on SPARC big-endian hardware. Includes an embedded unit test runner (`_TEST_MATH_COMPAT_CUSTOM`). |
| **`dns_rfc2553_compat.h`** | Modern RFC 2553 name resolution structure shims (`struct addrinfo`, `getaddrinfo`, `freeaddrinfo`, `gai_strerror`). |
| **`ftello_compat.h`** | POSIX file stream offset functions (`ftello`, `fseeko`). |
